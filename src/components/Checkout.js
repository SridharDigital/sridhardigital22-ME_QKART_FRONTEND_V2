...
const Checkout = () => {
    ...
    if (validateRequest(items, addresses)) {
      try {
        ...
        if (e.response) {
          ...
          enqueueSnackbar("Couldnot complete checkout", {
            variant: "error",
          });
        }
      }
  ...
  useEffect(() => {
    const onLoadHandler = async () => {
      ...
      const fetchedAddresses = getAddresses(token);

      if (fetchedAddresses) {
        setAddresses({ ...addresses, all: fetchedAddresses });
      }
    };
    onLoadHandler();
...
const OrderDetails = ({ items }) => {
  return (
    <Box className="cart" paddingY="1rem">
      <Typography color="#3C3C3C" variant="h5" padding="1rem" fontWeight="700">
        Order Details
      </Typography>
      <Box
        paddingY="0.5rem"
...
// curl 'http://13.127.80.236:8082/api/v1/user/addresses' \
//   -H 'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6InJlc3BvbnNlIiwiaWF0IjoxNzE3NzgyMzMyLCJleHAiOjE3MTc4MDM5MzJ9.BaLtKl4nKHQMyO3A9QB9ZkQRlSJWkHjNxrD2G-hf_zg'
...
// curl 'http://13.127.80.236:8082/api/v1/user/addresses/Skpe8XE2C6VpRdIHk8yOy' \
//   -X 'DELETE' \
